    ...
    class extends React.Component {
        ...
        render() {
            ...
            const formList = [
                ...
                { type: 'SELECT', label: '入库仓库', name: 'warehouseId', width: '100px', list: this.state.warehouseList, onChange: this.props.onChange },
                { type: 'SELECT', label: '入库类型', name: 'type', width: '100px', list: typeConfig[0] },
                            ...
                            if (_this.state.storageId) {/*更新库单*/
                                ...
                                Req.postStorageRecords({
                                    ...
                                    if (result) {
                                        ...
                                        _this.props.history.push(`/app/storehouseM/indata/add?id=${id}`); //地址栏改变
                                    }
                                })
                            }
                        }
            ...
            onOk() {
                Req.deleteStorageRecords(_this.state.storageId).then(result => {
                    if (result) {
                        ...
                        _this.props.history.push('/app/storehouseM/indata');//跳转到入库单首页
                    }
                })
            },
            onCancel() {
                        ...
                        if (!err) {
                            if (_this.state.storageId) {/*更新库单*/
                                Req.putStorageRecords({
                                    ...
                                    if (result) {
                                        message.success("更新成功 " + JSON.stringify(result.data))
                                        _this.props.history.push(`/app/storehouseM/indata/check?id=${_this.state.storageId}`);//跳转到入库单审核 
                                    }
                                })
                            } else {/*新增库单*/
                                Req.postStorageRecords({
                                    ...
                                    if (result) {
                                        let id = result.data;
                                        message.success("新增成功 " + JSON.stringify(result.data))
                                        _this.props.history.push(`/app/storehouseM/indata/check?id=${id}`); //地址栏改变
                                    }
                                })
                            }
                        }
        ...
        if (pass) {
            ...
            Req.putStorageRecordsStatus(this.state.storageId, "3").then(result => {
                if (result) {
                    ...
                    this.props.history.push(`/app/storehouseM/indata/detail?id=${this.state.storageId}`); //跳转"已审核"状态
                }
            })
        } else {
            console.log("变为'草稿'");
            Req.putStorageRecordsStatus(parseInt(this.state.storageId), "1").then(result => {
                if (result) {
                    ...
                    this.setState({ checkModal: false });
                    this.props.history.push(`/app/storehouseM/indata/add?id=${this.state.storageId}`); //跳转"草稿状态"
                }
            })
        }
    }
